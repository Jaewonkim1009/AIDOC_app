...
class AuthService {
  ...
  static Future<http.Response> deleteAccount(String email, String platform) async {
    ...
    final url = '$baseUrl/loginaccount/$encodedEmail?platform=$platform';
    print('[회원탈퇴 요청] email: $email, platform: $platform, url: $url');
    final response = await http.delete(
      Uri.parse(url),
      headers: {'Content-Type': 'application/json'},
    );
    print('[회원탈퇴 응답] statusCode: \\${response.statusCode}, body: \\${response.body}');
    return response;
  }
} 